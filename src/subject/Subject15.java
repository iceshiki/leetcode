package subject;

import java.io.Serializable;
import java.util.*;

/**
 * 三数之和
 * 给你一个整数数组 nums ，判断是否存在三元组 [nums[i], nums[j], nums[k]] 满足 i != j、i != k 且 j != k ，同时还满足 nums[i] + nums[j] + nums[k] == 0 。请
 * 你返回所有和为 0 且不重复的三元组。
 * 注意：答案中不可以包含重复的三元组。
 * 示例 1：
 * 输入：nums = [-1,0,1,2,-1,-4]
 * 输出：[[-1,-1,2],[-1,0,1]]
 * 解释：
 * nums[0] + nums[1] + nums[2] = (-1) + 0 + 1 = 0 。
 * nums[1] + nums[2] + nums[4] = 0 + 1 + (-1) = 0 。
 * nums[0] + nums[3] + nums[4] = (-1) + 2 + (-1) = 0 。
 * 不同的三元组是 [-1,0,1] 和 [-1,-1,2] 。
 * 注意，输出的顺序和三元组的顺序并不重要。
 * 示例 2：
 * 输入：nums = [0,1,1]
 * 输出：[]
 * 解释：唯一可能的三元组和不为 0 。
 * 示例 3：
 * 输入：nums = [0,0,0]
 * 输出：[[0,0,0]]
 * 解释：唯一可能的三元组和为 0 。
 * 提示：
 * 3 <= nums.length <= 3000
 * -105 <= nums[i] <= 105
 *
 * @author ice_shiki
 * @createDate 2022/11/25 13:30
 */
public class Subject15 {

    public List<List<Integer>> threeSum(int[] nums) {
        List<List<Integer>> tuplesList = new ArrayList<>();
        Arrays.sort(nums);
        if (nums[0] > 0) {
            // 第一个数大于0，不存在和为0的元祖
            return tuplesList;
        }
        int sum;
        int idxRight;
        int idxLeft;
        // -1 -1 -1 0 0 0 2
        for (int i = 0; i < nums.length - 2; i++) {
            if (nums[i] > 0) {
                break;
            }
            if (i > 0 && nums[i] == nums[i - 1]) {
                continue;
            }
            idxLeft = i + 1;
            idxRight = nums.length - 1;
            while (idxLeft < idxRight) {
                sum = nums[i] + nums[idxLeft] + nums[idxRight];
                if (sum == 0) {
                    tuplesList.add(Arrays.asList(nums[i], nums[idxLeft], nums[idxRight]));
                    while (idxLeft < idxRight && nums[idxLeft] == nums[idxLeft + 1]) {
                        idxLeft++;
                    }
                    while (idxLeft < idxRight && nums[idxRight] == nums[idxRight - 1]) {
                        idxRight--;
                    }
                    idxLeft += 1;
                } else if (sum > 0) {
                    idxRight -= 1;
                } else {
                    idxLeft += 1;
                }
            }
        }
        return tuplesList;
    }

    public static void main(String[] args) {
//        int[] nums = {-1,0,1,2,-1,-4};
//        int[] nums = {0,1,1};
        int[] nums = {0, 0, 0, 0};
        List<List<Integer>> lists = new Subject15().threeSum(nums);
    }
}
